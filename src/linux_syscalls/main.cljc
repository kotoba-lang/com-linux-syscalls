(ns linux_syscalls.main
  "Kotodama WASM entrypoint for the Linux_syscalls clean-room actor (L4) — Clojure port.
  ...
  facts (`linux_syscalls.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "linux_syscalls")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"linux_syscalls.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "linux_syscalls-compat" :tier tier :entities entities} 200])
...
(defn start! [] :linux_syscalls-compat/ready)
